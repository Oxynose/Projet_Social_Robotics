...
# PPO
# ---------------------------
def play_ppo(model_path, deterministic=True, sleep=0.0):
    env = gym.make("CarRacing-v2", render_mode="human")
    obs, info = env.reset()
...
def train_ppo(total_timesteps=200_000, rollout_steps=2048, update_epochs=8, minibatch_size=64, save_path=MODEL_PATH_PPO):
    ...
    while timestep < total_timesteps:
        ...
        for step in range(rollout_steps):
            ...
            if timestep >= total_timesteps: break
    ...
    env.close()
    print("PPO training complete. Model saved to", save_path)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--play", action="store_true", help="Play using the PPO model")
    ...
    if args.train:
        train_ppo(total_timesteps=200_000, save_path=model_path)
    elif args.play:
        play_ppo(model_path)
    else:
        play_with_keyboard()
